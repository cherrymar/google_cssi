...
class WandHandler( webapp2.RequestHandler):
    def get(self):
        hogwarts_wands = Wand.query().order(Wand.key).fetch()
        start_template = jinja_env.get_template("templates/wandlist.html")
        self.response.write(start_template.render({'wand_info' : hogwarts_wands}))
